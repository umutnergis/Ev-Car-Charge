...
//#define DEBUG
#define check10A 0x22
#define check16A 0x23
#define check24A 0x24
#define check32A 0x25
...
void thread1(void *pvParameters) {
  for (;;) {
    ...
    kwh = (((230 * current1 * 0.9) + (230 * current2 * 0.9) + (230 * current3 * 0.9))/1000);
  
    if (Serial.available()) {
      byte incomingByte = Serial.read();

      char hexValue[3];
      sprintf(hexValue, "%02X", incomingByte);
      #ifdef DEBUG
...
/* void switchToPage(uint16_t pageNumber)
{
  uint8_t command[] = {0x5A, 0xA5, 0x07, 0x82, 0x00, 0x84, 0x5A, 0x01, (uint8_t)(pageNumber >> 8), (uint8_t)pageNumber};
